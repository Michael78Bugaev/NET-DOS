...
void tts_in(uint32_t num, uint16_t ss0, uint32_t esp0){
    uint32_t base = (uint32_t) &tss_entry;
    uint32_t limit = base + sizeof(tss_entry);

    gdt_gate(num, base, limit, 0xE9, 0x00);
    memcpy(0, &tss_entry, sizeof(tss_entry));
...
void gdt_gate(uint32_t num, uint32_t base, uint32_t limit, uint8_t access, uint8_t gran){
